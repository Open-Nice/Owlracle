...
const CourseEvalComponent = ({ text, dic }) => (
  <tr className="align-middle">
    ...
    <td
      className={
        dic['Class Mean'] <= dic['Rice Mean'] ? 'rate-better' : 'rate-worse'
      }
    >
      {dic['Class Mean']}
    </td>
        ...
        ? [
            ...new Set(
              comments
                .filter(
                  comment =>
                    `${comment.year} ${comment.semester}` === yearSemester
                )
  ...
  const yearSemesters = useMemo(
    () =>
      instructor !== '0'
        ...
        : [
            ...new Set(
              comments.map(comment => `${comment.year} ${comment.semester}`)
            )
          ],
  ...
  const score = useMemo(
    () =>
      yearSemester !== '0' && instructor !== '0'
        ? scores.filter(
            score =>
              score.instructor === instructor &&
              `${score.year} ${score.semester}` == yearSemester
          )[0]
        : null,
    ...
    <div>
      <div className="flex items-center">
        <select
          ...
          value={yearSemester}
          className="classcard-dropdown shadow"
        >
          ...
          {yearSemesters.map((yearSemester, index) => {
            return (
              <option
                key={index}
                value={yearSemester}
                className="classcard-dropdown-item"
              >
                {yearSemester}
              </option>
            )
        ...
        <div className="tooltip">
          <button
            ...
            className="color-mid-purple cursor-pt"
          >
            <RefreshRoundedIcon color="inherit" />
          </button>
          <span className="tooltiptext tooltip-reset bg-white text-black shadow">
            Reset
          </span>
        </div>
      </div>
        ...
        <>
            ...
            <div style={{ paddingLeft: '5px' }}>
              <div className="color-mid-grey">
                ...
                <span className="color-red">
                  lower than average, heavier workload
                </span>
              </div>
              ...
              <table className="classcard-table">
                ...
                <tbody>
                  <CourseEvalComponent
                    ...
                    dic={score.organization}
                  />
                  <CourseEvalComponent
                    text={'Assignment Quality:'}
                    dic={score.assignment}
                  />
                  <CourseEvalComponent
                    text={'Overall Rating:'}
                    dic={score.overall_quality}
                  />
                  <CourseEvalComponent
                  ...
                  <CourseEvalComponent
                    text={'Expected grade:'}
                    dic={score.expected_grade}
                  />
                  <CourseEvalComponent
                    text={'Expected P&F:'}
                    dic={score.expectedPF}
                  />
                </tbody>
              </table>
              ...
              >
                ...
                <button
                  className="comment-type-btn color-mid-grey shadow"
                  onClick={e => {
                    handleCheckComment(e, 0)
                  }}
                >
                  <SearchRoundedIcon />
                  See comment <span className="color-green">positivity</span>/
                  <span className="color-red">negativity</span>
                </button>
              </div>
                    ...
                    <div key={idx} className="p-3">
                      <div className="student-comment">
                        {checkComments[0] ? (
                          <div
                            className={twMerge(
                              'student-comment-judgement',
                              sentiment.analyze(comment).score > 0
                                ? 'bg-green'
                                : sentiment.analyze(comment).score < 0
                                ? 'bg-red'
                                : 'bg-mid-grey'
          ...
          {score.instructor_evaluations.map((instructor_evaluation, index) => (
            <div key={index} className="mt-4">
              <b>
                Instructor Evaluation: {instructor_evaluation['instructor']}
              </b>
              <div style={{ paddingLeft: '5px' }}>
                <div>
                  {' '}
                ...
                <table className="classcard-table">
                  ...
                  <tbody>
                    <InstructorEvalComponent
                      text={'Organization:'}
                      dic={instructor_evaluation['Organization']}
                    />
                    <InstructorEvalComponent
                      text={'Presentation:'}
                      dic={instructor_evaluation['Presentation']}
                    />
                    <InstructorEvalComponent
                      text={'Responsiveness:'}
                      dic={instructor_evaluation['Responsiveness']}
                    />
                    <InstructorEvalComponent
                      text={'Class Atmosphere:'}
                      dic={instructor_evaluation['Class Atmosphere']}
                    />
                    <InstructorEvalComponent
                      text={'Independence:'}
                      dic={instructor_evaluation['Independence']}
                    />
                    <InstructorEvalComponent
                      text={'Stimulation:'}
                      dic={instructor_evaluation['Stimulation']}
                    />
                    <InstructorEvalComponent
                      text={'Knowledge:'}
                      dic={instructor_evaluation['Knowledge']}
                    />
                    <InstructorEvalComponent
                      text={'Effectiveness:'}
                      dic={instructor_evaluation['Effectiveness']}
                    />
                    <InstructorEvalComponent
                      text={'Responsibility:'}
                      dic={instructor_evaluation['Responsibility']}
                    />
                  </tbody>
                </table>
                ...
                <div className="student-comment-frame">
                  <div className="student-comment-scroll-wrapper">
                    {comment.instructor_comments[index].comments.map(
                      (comment, idx) => (
                        <div key={idx} className="p-3">
                          <div className="student-comment">
                            {checkComments[0] ? (
